...
rm(list=ls()) # this eliminates all objects from R environment

# important option for text analysis (background: we will import data first in R's native dataframe format and reshape it into corpus / corpora and other object types. We will, however keep "text" as "text" so far)

options(stringsAsFactors = F)

# check working directory. It should be the destination folder of the extracted zip file. If necessary, use `setwd("your-tutorial-folder-path")` to change it.

setwd("C:/Users/FritscheU/Dropbox/RCourse_Text_Mining/Code")
#setwd("C:/Users/Ulrich Fritsche/Dropbox/RCourse_Text_Mining/Code")

getwd() # check that the folder is correct

# Be aware that all necessary packages are installed
# eventually you have to use options(download.file.method = "libcurl")
...
write.csv2(all_articles, file = "data/guardian_merkel.csv") # write.csv2 uses semikolon separated columns and "," as a decimal 

# Exercise 3: 
# Extract text from various file formats
# PDF from folder

data_files <- list.files(path = "data/documents", full.names = T, recursive = T) # full name preserved, 
