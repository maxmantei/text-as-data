...
source("code/F.measure.R") ## here c-optimisation is included

textdata <- read.csv("code/data/sotu_paragraphs.csv", sep = ";", encoding = "UTF-8")
english_stopwords <- readLines("code/resources/stopwords_en.txt", encoding = "UTF-8")

# Create corpus object
names(textdata)[names(textdata) == "p_id"] <- "doc_id"
textdata <- textdata[c(1,6,2,3,4,5)]
...
trainingData <- read.csv2("code/data/paragraph_training_data_format.csv", stringsAsFactors = T)
...
as.character(processedCorpus[[domestic_example]])

foreign_example <- sample(trainingData$ID[trainingData$LABEL == "FOREIGN"], 1)
as.character(processedCorpus[[foreign_example]])
...
# First, we load the packages. Since Liblinear requires a special Sparse Matrix format, 
# we also load the “SparseM” package and a conversion function which allows to convert slam-matrices (as used in the tm package) into SparseM-matrices.
# Then, we split the annotated data into a training set (80%) and a test set (20%) using a boolean selector. 
# The expression assigned to selector_idx creates a boolean vector of length 300 containing a FALSE value in every fifths position. 
# This selector is used to select to training set. Its inverted vector (!) is used to select the test set.
annotatedDTM <- DTM[trainingData[, "ID"], ]
annotatedDTM <- convertSlamToSparseM(annotatedDTM)
annotatedLabels <- trainingData[, "LABEL"]

# split into training and test set
# rep() replicates, length.out definiert Länge, generiert eine Dummy-Variable mit 111101111011110...
...
# The model created by the LiblineaR function can now be utilized to predict the labels of the test set. 
# Then we compare the result of the automatic classification to our known labels to determine the accuracy of the process.
...
# If classes in training/test sets are imbalanced, accuracy might be a misleading measurement. 
# Other measure should be considered additionally. To utilize accuracy and F-measure in a meaningful way, 
# the less frequent class should be defined as POSITIVE class (FOREIGN in our case).
...
for (j in 1:k) {
  ...
  # create model
  model <- LiblineaR(foldDTM, foldLabels, type = 2)
